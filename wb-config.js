...
(function(){
  ...
  function boot(){
    ...
    function addWidget(p,w){grid.appendChild(p);p.style.position='relative';var it={p:p,w:w||2,cs:1,rs:1,h:2};W.push(it);
      ...
      var x=document.createElement('button');x.className='pw-x';x.textContent='\u00d7';x.onclick=function(e){e.stopPropagation();it._widx=W.indexOf(it);W=W.filter(function(z){return z!==it;});it._name=pwPanelName(p);HIDDEN.push(it);p.style.display='none';autoPack();applyLayout();};p.appendChild(x);
