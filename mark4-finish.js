...
(function(){
  ...
  function hideDepartmentSelector(){var sel=document.querySelector('.myres-picks select:not(#heroHomeFilter)');if(!sel)return;var p=getProfile();var email=p?String(p.email||'').trim().toLowerCase():'';if(email==='mshilcrat@hasccenter.org'){sel.style.display='';return;}sel.style.display='none';}
